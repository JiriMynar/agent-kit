...
export function AgentKitPanel({
    ...
    if (!checkAvailability()) {
      ...
      timeoutId = window.setTimeout(() => {
        if (!checkAvailability() && isMountedRef.current) {
          console.error(
            "AgentKit web component is unavailable. Verify that the script URL is reachable."
          );
          setErrorState({
            script:
              "Error: AgentKit web component is unavailable. Verify that the script URL is reachable.",
            retryable: false,
          });
  ...
  const isWorkflowConfigured = Boolean(
    AGENTKIT_WORKFLOW_ID && !AGENTKIT_WORKFLOW_ID.startsWith("wf_replace")
  );
  ...
  useEffect(() => {
    if (!isWorkflowConfigured && isMountedRef.current) {
      setErrorState({
        session:
          "Set NEXT_PUBLIC_AGENTKIT_WORKFLOW_ID in your .env.local file or update AGENTKIT_WORKFLOW_ID in lib/config.ts.",
        retryable: false,
      });
      setIsInitializingSession(false);
    }
  ...
  const getClientSecret = useCallback(
    async (currentSecret: string | null) => {
      if (isDev) {
        console.info("[AgentKitPanel] getClientSecret invoked", {
          currentSecretPresent: Boolean(currentSecret),
          workflowId: AGENTKIT_WORKFLOW_ID,
      ...
      try {
        const response = await fetch(CREATE_SESSION_ENDPOINT, {
          ...
          body: JSON.stringify({
            workflow: { id: AGENTKIT_WORKFLOW_ID },
        ...
        if (isDev) {
          console.info("[AgentKitPanel] createSession response", {
        ...
        const detail =
          error instanceof Error
            ? error.message
            : "Unable to start AgentKit session.";
        if (isMountedRef.current) {
          setErrorState({ session: detail, retryable: false });
        }
    ...
    onClientTool: async (invocation: {
      ...
      if (invocation.name === "switch_theme") {
        ...
        if (requested === "light" || requested === "dark") {
          if (isDev) {
            console.debug("[AgentKitPanel] switch_theme", requested);
          }
  ...
  if (isDev) {
    console.debug("[AgentKitPanel] render state", {
